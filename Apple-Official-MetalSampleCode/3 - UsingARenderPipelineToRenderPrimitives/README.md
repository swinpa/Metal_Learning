...
This sample focuses on the three main stages(阶段) of the pipeline: the vertex stage, the rasterization stage, and the fragment stage.
The vertex stage and fragment stage are programmable(可编程的), so you write functions for them in Metal Shading Language (MSL).
The rasterization stage has fixed behavior(栅格化阶段具有固定的行为).
...
Rendering starts with a drawing command, which includes a vertex count and what kind of primitive(图元) to render. For example, here's the drawing command from this sample:
...
The vertex stage provides data for each vertex. When enough vertices have been processed, the render pipeline rasterizes the primitive, determining which pixels in the render targets lie within the boundaries of the primitive(确定渲染目标中的哪些像素位于图元的边界内). The fragment stage determines the values to write into the render targets for those pixels(片段阶段确定要写入这些像素的渲染目标的值。).
...
A vertex function generates data for a single vertex and a fragment function generates data for a single fragment(顶点函数为单个顶点生成数据，片段函数为单个片段生成数据), but you decide how they work.
